// Cross-Domain Concept Mapping Service
// Identifies structural similarities between concepts across different domains
...
export interface ConceptAnalogy {
  ...
  structuralSimilarity: string; // Explanation of why they're similar
  keyDifferences: string; // Important distinctions
  relevanceScore: number; // 1-100
}

// Pre-defined cross-domain concept mappings
export const crossDomainMappings: Record<string, ConceptAnalogy[]> = {
  'invisible hand': [
    {
      id: 'map-1',
...
// Function to find concept mappings
export function findConceptMappings(concept: string, userBookIds: string[]): ConceptAnalogy[] {
  const normalizedConcept = concept.toLowerCase().trim();
  
  // Find exact match
  let analogies = crossDomainMappings[normalizedConcept] || [];
  
  // If no exact match, try partial matches
  if (analogies.length === 0) {
    for (const [key, mappings] of Object.entries(crossDomainMappings)) {
  ...
  // Filter to user's books (if userBookIds provided) and sort by relevance
  if (userBookIds.length > 0) {
    analogies = analogies.filter(a => 
      userBookIds.includes(a.targetBookId) || a.targetBookId === 'news'
    );
  }
  
  return analogies.sort((a, b) => b.relevanceScore - a.relevanceScore).slice(0, 3);
}

// Function to get all available concept mappings for a book
export function getConceptsForBook(bookId: string): string[] {
  const concepts: string[] = [];
  
  for (const [concept, mappings] of Object.entries(crossDomainMappings)) {
    if (mappings.some(m => m.targetBookId === bookId)) {
...
// Generate concept mapping using AI
export async function generateConceptMapping(
  sourceConcept: string,
  sourceDomain: string,
  sourceContext: string,
  targetBooks: { id: string; title: string; domain: string }[]
): Promise<ConceptAnalogy[]> {
  const OPENAI_API_KEY = (import.meta as any).env?.VITE_OPENAI_API_KEY || '';
  
  if (!OPENAI_API_KEY || targetBooks.length === 0) {
    return findConceptMappings(sourceConcept, targetBooks.map(b => b.id));
  }
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `You are a cross-domain concept mapper. Your task is to find structural similarities between concepts from different domains.

Given a concept from one domain, identify analogous concepts in other domains based on underlying patterns, mechanisms, or structures—not just keywords.

Respond in JSON format with an array of analogies:
[
  {
    ...
    "relevanceScore": 85
          ...
          {
            role: 'user',
            content: `Source Concept: "${sourceConcept}" from ${sourceDomain}
Context: ${sourceContext}

Find structural analogies in these books:
${targetBooks.map(b => `- ${b.title} (${b.domain})`).join('\n')}

Generate 2-3 cross-domain analogies.`
          }
        ],
        temperature: 0.7,
        max_tokens: 1500,
    ...
    if (jsonMatch) {
      ...
      return analogies.map((a: any, i: number) => ({
        id: `ai-map-${Date.now()}-${i}`,
        ...a
      }));
    }
